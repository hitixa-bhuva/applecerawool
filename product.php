<?php
$Title = "Products | Apple Cerawool Industrial Insulation & Refractory Solutions";
$MetaDescription = "Explore Apple Cerawool's wide range of industrial insulation and refractory products including ceramic fiber boards, blankets, modules, papers, textiles, gaskets, coatings, and furnace doors.";
$MetaKeywords = "Apple Cerawool products, industrial insulation, refractory solutions, ceramic fiber boards, ceramic blankets, ceramic modules, ceramic papers, ceramic textiles, gaskets, coatings, furnace doors";
?>

<?php
include __DIR__ . '/A_Layout/Header/header.php';
?>

    <!-- Offcanvas area start -->
    <div class="fix">
        <div class="offcanvas-area" data-lenis-prevent>
            <div class="offcanvas-wrapper">
                <div class="offcanvas-content">
                    <div class="offcanvas-top d-flex justify-content-between align-items-center mb-20">
                        <div class="offcanvas-logo">
                            <a class="logo-black" href="home"><img src="assets/images/logo/apple-cera-wool-or-blu.png" alt="logo"></a>
                        </div>
                        <div class="offcanvas-close">
                            <button class="offcanvas-close-icon animation--flip">
                                <span class="offcanvas-m-lines">
                                <span class="offcanvas-m-line line--1"></span><span
                                    class="offcanvas-m-line line--2"></span><span
                                    class="offcanvas-m-line line--3"></span>
                                </span>
                            </button>
                        </div>
                    </div>
                    <div class="offcanvas-about mb-30 d-none d-sm-block">
                        <p> Apple Cera Wool provides advanced ceramic fiber and refractory insulation solutions, delivering quality, safety, and performance for industries worldwide since 2025.
                        </p>
                    </div>
                    <div class="offcanvas-gallery d-none d-sm-block">
                        <div class="offcanvas-gallery-thumb-wrapper">
                            <div class="offcanvas-popup-thumb">
                                <a class="popup-image" href="assets/images/product/CeramicFiberBlanket/Ceramic-Fiber-Blanket-3.jpg">
                                    <img src="assets/images/product/CeramicFiberBlanket/Ceramic-Fiber-Blanket-3.jpg" alt="image">
                                </a>
                            </div>
                            <div class="offcanvas-popup-thumb">
                                <a class="popup-image" href="assets/images/product/CeramicFiberBoard/Ceramic-Fiber-Board-5.jpg">
                                    <img src="assets/images/product/CeramicFiberBoard/Ceramic-Fiber-Board-5.jpg" alt="image">
                                </a>
                            </div>
                            <div class="offcanvas-popup-thumb">
                                <a class="popup-image" href="assets/images/product/ceramicfiberbulk/ceramic-fiber-bulk-4.jpg">
                                    <img src="assets/images/product/ceramicfiberbulk/ceramic-fiber-bulk-4.jpg" alt="image">
                                </a>
                            </div>
                            <div class="offcanvas-popup-thumb">
                                <a class="popup-image" href="assets/images/product/CeramicFiberModule/Ceramic-Fiber-Module-6.jpg">
                                    <img src="assets/images/product/CeramicFiberModule/Ceramic-Fiber-Module-6.jpg" alt="image">
                                </a>
                            </div>
                            <div class="offcanvas-popup-thumb">
                                <a class="popup-image" href="assets/images/product/CeramicFiberPaper/Ceramic-Fiber-Paper.jpg">
                                    <img src="assets/images/product/CeramicFiberPaper/Ceramic-Fiber-Paper.jpg" alt="image">
                                </a>
                            </div>
                            <div class="offcanvas-popup-thumb">
                                <a class="popup-image" href="assets/images/product/CeramicFiberRope/Ceramic-Fiber-Rope-8.jpg">
                                    <img src="assets/images/product/CeramicFiberRope/Ceramic-Fiber-Rope-8.jpg" alt="image">
                                </a>
                            </div>
                        </div>
                    </div>
                    <div class="mobile-menu">
                        <div class="rs-offcanvas-menu mb-25">
                            <nav></nav>
                        </div>
                    </div>
                    <div class="offcanvas-contact mb-30">
                        <h4 class="offcanvas-title-meta">Contact Info</h4>
                        <ul>
                            <li class="d-flex align-items-center gap-15">
                                <div class="offcanvas-contact-icon">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="18" viewBox="0 0 14 18" fill="none">
                                        <path d="M11.8768 9.68475C11.3059 10.835 10.5331 11.9818 9.74128 13.0109C8.95198 14.0368 8.15999 14.9249 7.5643 15.5572C7.48514 15.6412 7.40956 15.7206 7.33802 15.7951C7.26648 15.7206 7.1909 15.6412 7.11174 15.5572C6.51605 14.9249 5.72406 14.0368 4.93476 13.0109C4.14299 11.9818 3.37019 10.835 2.79925 9.68475C2.22242 8.52266 1.89032 7.43373 1.89032 6.5C1.89032 3.50846 4.32934 1.08333 7.33802 1.08333C10.3467 1.08333 12.7857 3.50846 12.7857 6.5C12.7857 7.43373 12.4536 8.52266 11.8768 9.68475ZM7.33802 17.3333C7.33802 17.3333 13.8753 11.1732 13.8753 6.5C13.8753 2.91015 10.9484 0 7.33802 0C3.7276 0 0.800781 2.91015 0.800781 6.5C0.800781 11.1732 7.33802 17.3333 7.33802 17.3333Z" fill="#6D6D6D"></path>
                                        <path d="M7.33802 8.66667C6.13455 8.66667 5.15894 7.69662 5.15894 6.5C5.15894 5.30338 6.13455 4.33333 7.33802 4.33333C8.54149 4.33333 9.5171 5.30338 9.5171 6.5C9.5171 7.69662 8.54149 8.66667 7.33802 8.66667ZM7.33802 9.75C9.14323 9.75 10.6066 8.29492 10.6066 6.5C10.6066 4.70507 9.14323 3.25 7.33802 3.25C5.53281 3.25 4.0694 4.70507 4.0694 6.5C4.0694 8.29492 5.53281 9.75 7.33802 9.75Z" fill="#6D6D6D"></path>
                                    </svg>
                                </div>
                                <div class="offcanvas-contact-text">
                                    <a target="_blank" href="https://www.google.com/maps?q=14+Paiki+3,+Adalshar+Road,+Lakhatar,+Surendranagar,+Gujarat+363775">
                                                    14 Paiki 3, Adalshar Road, Lakhatar, Surendranagar 363775
                                                </a>
                                </div>
                            </li>
                            <li class="d-flex align-items-center gap-15">
                                <div class="offcanvas-contact-icon">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                                        <path d="M3.65387 1.32849C3.40343 1.00649 2.92745 0.976861 2.639 1.26531L1.60508 2.29923C1.1216 2.78271 0.94387 3.46766 1.1551 4.06847C2.00338 6.48124 3.39215 8.74671 5.32272 10.6773C7.25329 12.6078 9.51876 13.9966 11.9315 14.8449C12.5323 15.0561 13.2173 14.8784 13.7008 14.3949L14.7347 13.361C15.0231 13.0726 14.9935 12.5966 14.6715 12.3461L12.3653 10.5524C12.2008 10.4245 11.9866 10.3793 11.7845 10.4298L9.59541 10.9771C9.00082 11.1257 8.37183 10.9515 7.93845 10.5181L5.48187 8.06155C5.04849 7.62817 4.87427 6.99919 5.02292 6.40459L5.57019 4.21553C5.62073 4.01336 5.57552 3.79918 5.44758 3.63468L3.65387 1.32849ZM1.88477 0.511076C2.62689 -0.231039 3.8515 -0.154797 4.49583 0.673634L6.28954 2.97983C6.6187 3.40304 6.73502 3.95409 6.60498 4.47423L6.05772 6.66329C5.99994 6.8944 6.06766 7.13888 6.2361 7.30732L8.69268 9.7639C8.86113 9.93235 9.1056 10.0001 9.33671 9.94229L11.5258 9.39502C12.0459 9.26499 12.597 9.3813 13.0202 9.71047L15.3264 11.5042C16.1548 12.1485 16.231 13.3731 15.4889 14.1152L14.455 15.1492C13.7153 15.8889 12.6089 16.2137 11.5778 15.8512C9.01754 14.9511 6.61438 13.4774 4.56849 11.4315C2.5226 9.38562 1.04895 6.98246 0.148838 4.42225C-0.213682 3.39112 0.11113 2.28472 0.85085 1.545L1.88477 0.511076Z" fill="#6D6D6D"></path>
                                        <path d="M11 0.5C11 0.223858 11.2239 0 11.5 0H15.5C15.7761 0 16 0.223858 16 0.5V4.5C16 4.77614 15.7761 5 15.5 5C15.2239 5 15 4.77614 15 4.5V1.70711L10.8536 5.85355C10.6583 6.04882 10.3417 6.04882 10.1464 5.85355C9.95118 5.65829 9.95118 5.34171 10.1464 5.14645L14.2929 1H11.5C11.2239 1 11 0.776142 11 0.5Z" fill="#6D6D6D"></path>
                                    </svg>
                                </div>
                                <div class="offcanvas-contact-text">
                                    <a href="tel:+919879595007">+91 98795 95007</a> 
<a href="tel:918000002234">+91 80000 02234</a>
                                </div>
                            </li>
                            <li class="d-flex align-items-center gap-15">
                                <div class="offcanvas-contact-icon">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                                        <path d="M2 2C0.895431 2 0 2.89543 0 4V12L2.58386e-05 12.0103C0.00555998 13.1101 0.898859 14 2 14H7.5C7.77614 14 8 13.7761 8 13.5C8 13.2239 7.77614 13 7.5 13H2C1.53715 13 1.14774 12.6855 1.03376 12.2586L6.67417 8.7876L8 9.5831L15 5.3831V8.5C15 8.77614 15.2239 9 15.5 9C15.7761 9 16 8.77614 16 8.5V4C16 2.89543 15.1046 2 14 2H2ZM5.70808 8.20794L1 11.1052V5.3831L5.70808 8.20794ZM1 4.2169V4C1 3.44772 1.44772 3 2 3H14C14.5523 3 15 3.44772 15 4V4.2169L8 8.4169L1 4.2169Z" fill="#6D6D6D"></path>
                                        <path d="M14.2467 14.2686C15.2567 14.2686 15.8339 13.4116 15.8339 12.2442V12.0344C15.8339 10.4297 14.6402 9 12.5197 9H12.4847C10.421 9 9 10.3598 9 12.4322V12.6465C9 14.8195 10.4385 16 12.3579 16H12.4016C12.9963 16 13.4204 15.9257 13.639 15.8251V15.0949C13.3941 15.2042 12.9656 15.2742 12.4585 15.2742H12.4147C11.0812 15.2742 9.84385 14.4872 9.84385 12.6202V12.4628C9.84385 10.8057 10.9019 9.73891 12.4847 9.73891H12.524C14.0587 9.73891 15.0075 10.7883 15.0075 12.065V12.183C15.0075 13.158 14.6839 13.5734 14.3691 13.5734C14.1374 13.5734 13.9582 13.4247 13.9582 13.1537V10.9631H13.0531V11.5315H13.0225C12.9394 11.2342 12.6552 10.9019 12.0693 10.9019C11.2911 10.9019 10.8101 11.4572 10.8101 12.3011V12.8301C10.8101 13.722 11.2998 14.2642 12.0693 14.2642C12.5415 14.2642 12.9656 14.0369 13.0837 13.6215H13.1274C13.2455 14.0412 13.7439 14.2686 14.2467 14.2686ZM11.7939 12.6814V12.4541C11.7939 11.9076 12.0212 11.6627 12.3666 11.6627C12.664 11.6627 12.9394 11.8551 12.9394 12.371V12.7383C12.9394 13.3111 12.6858 13.4816 12.3754 13.4816C12.0212 13.4816 11.7939 13.2673 11.7939 12.6814Z" fill="#6D6D6D"></path>
                                    </svg>
                                </div>
                                <div class="offcanvas-contact-text">
                                    <a href="mailto:apple.cerawool@gmail.com">apple.cerawool@gmail.com</a>
                                </div>
                            </li>
                        </ul>
                    </div>
                    <div class="offcanvas-social">
                        <h4 class="offcanvas-title-meta">Subscribe & Follow</h4>
                        <ul>
                            <li><a href="#"><i class="fab fa-facebook-f"></i></a></li>
                            <li><a href="#"><i class="fab fa-twitter"></i></a></li>
                            <li><a href="#"><i class="fab fa-youtube"></i></a></li>
                            <li><a href="#"><i class="fab fa-linkedin"></i></a></li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="offcanvas-overlay"></div>
    <div class="offcanvas-overlay-white"></div>
    <!-- Offcanvas area start -->

    <!-- Body main wrapper start -->
    <main>

        <!-- breadcrumb area start -->
        <section class="rs-breadcrumb-area rs-breadcrumb-one p-relative">
            <div class="rs-breadcrumb-bg" data-background="assets/images/gallery/gallery-thumb-01.png"></div>
            <div class="container">
                <div class="row">
                    <div class="col-xxl-6 col-xl-8 col-lg-8">
                        <div class="rs-breadcrumb-content-wrapper">
                            <div class="rs-breadcrumb-title-wrapper">
                                <h1 class="rs-breadcrumb-title">Products</h1>
                            </div>
                            <div class="rs-breadcrumb-menu">
                                <nav>
                                    <ul>
                                        <li><span><a href="home">Home</a></span></li>
                                        <li><span>Products</span></li>
                                    </ul>
                                </nav>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!-- breadcrumb area end -->

                <!-- why choose area start -->
        <section id="homeportfolio" class="rs-why-choote-area section-space-top  rs-why-choose-two">
            <div class="container">
                <div class="row g-5">
                    <div class="col-xl-6 col-lg-6">
                        <div class="rs-why-choose-section-area">
                            <div class="rs-section-title-wrapper">
                                <span class="rs-section-subtitle has-theme-yellow justify-content-start">
                           <svg xmlns="http://www.w3.org/2000/svg" width="15" height="28" viewBox="0 0 15 28"
                              fill="none">
                              <path
                                 d="M12.105 0.329612C12.4307 -0.0575748 13.0084 -0.112887 13.4018 0.212841C13.7889 0.538569 13.8443 1.11628 13.5185 1.50961L3.09523 13.9487L8.06105 13.3956C8.565 13.3403 9.0198 13.7029 9.07511 14.2069C9.09354 14.3728 9.06282 14.5326 9.00136 14.6801L3.62377 26.8365C3.42096 27.3036 2.87398 27.5126 2.41304 27.3097C1.94596 27.1069 1.737 26.56 1.93982 26.099L6.67209 15.4053L1.01794 16.0383C0.778256 16.0629 0.526278 15.9953 0.329612 15.8294C-0.0575748 15.5036 -0.112887 14.9259 0.212841 14.5326L12.1111 0.335757L12.105 0.329612Z"
                                 fill="#EA5501"></path>
                              <path
                                 d="M11.9702 0.550616C12.173 0.0835344 12.72 -0.125423 13.1809 0.0773886C13.648 0.2802 13.857 0.827178 13.6541 1.28811L9.70238 10.2733L14.0598 10.1442C14.5637 10.1319 14.9878 10.5314 15.0001 11.0354C15.0062 11.2443 14.9386 11.4471 14.8218 11.6008L3.52584 26.9961C3.22469 27.4078 2.65313 27.4939 2.24136 27.1927C1.82959 26.8916 1.74355 26.32 2.0447 25.9083L12.2283 12.0249L8.36259 12.1416C8.20895 12.1539 8.0553 12.1355 7.9078 12.0679C7.44072 11.8651 7.23176 11.3181 7.43458 10.8571L11.9763 0.538325L11.9702 0.550616Z"
                                 fill="#EA5501"></path>
                           </svg>
                               Why Choose Our Product
                            </span>
                                <h2 class="rs-section-title rs-split-text-enable split-in-fade">What it is & why it matters</h2>
                                <p>The Ceramic Fiber Blanket from Apple Cera Wool is a flexible, lightweight insulation material designed for temperatures up to 1600°C. It ensures thermal efficiency, protects equipment, and enhances operational safety across industrial environments.</p>
                            </div>
                            <div class="rs-why-choose-thumb-wrapper">
                                <div class="rs-why-choose-thumb image-one">
                                    <img src="assets/images/why-choose/why-choose-thumb-05.png" alt="image">
                                </div>
                                <div class="rs-why-choose-thumb image-two">
                                    <img src="assets/images/why-choose/why-choose-thumb-06.png" alt="image">
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-6 col-lg-6">
                        <div class="rs-why-choose-content-wrapper">
                            <div class="rs-counter-item">
                                <div class="rs-counter-number-wrapper">
                                    <span class="rs-counter-number odometer" data-count="5">00</span>
                                    <span class="prefix">+</span>
                                </div>
                                <span class="rs-counter-title">Years of Experience</span>
                            </div>
                            <div class="rs-counter-item">
                                <div class="rs-counter-number-wrapper">
                                    <span class="rs-counter-number odometer" data-count="35">00</span>
                                    <span class="prefix">%</span>
                                </div>
                                <span class="rs-counter-title">Clients Satisfaction</span>
                            </div>
                            <div class="rs-counter-item">
                                <div class="rs-counter-number-wrapper">
                                    <span class="rs-counter-number odometer" data-count="30">00</span>
                                    <span class="prefix">+</span>
                                </div>
                                <span class="rs-counter-title">Projects Completed</span>
                            </div>
                            <div class="rs-counter-item">
                                <div class="rs-counter-number-wrapper">
                                    <span class="rs-counter-number odometer" data-count="30">00</span>
                                    <span class="prefix">+</span>
                                </div>
                                <span class="rs-counter-title">Special Machinery</span>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!-- why choose area end -->

                <!-- category area start -->
        <section class="rs-category-area rs-category-one section-space primary-bg">
            <div class="rs-category-bg-thumb" data-background="assets/images/bg/category-bg-01.png"></div>
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-xl-8 col-lg-9 col-md-10">
                        <div class="rs-section-title-wrapper text-center section-title-space">
                            <span class="rs-section-subtitle has-theme-blue">
                        Key Features & Benefits    
                     </span>
                            <h2 class="rs-section-title has-theme-blue rs-split-text-enable split-in-fade">What makes it exceptional</h2>
                        </div>
                    </div>
                </div>
                <div class="row  process-counts g-5">
                    <div class="col-xl-4 col-lg-4 col-md-6">
                        <div class="rs-category-item wow fadeIn" data-wow-delay=".3s">
                            <div class="rs-category-icon">
                                <svg xmlns="http://www.w3.org/2000/svg" id="Layer_1" viewBox="0 0 500 500">
                                    <g>
                                        <path d="m274.648 446.616h-228.847c-3.023 0-5.737-1.855-6.831-4.672s-.35-6.017 1.878-8.056l54.999-50.424c1.351-1.243 3.12-1.93 4.952-1.93h3.183l19.324-105.858h-27.184c-4.048 0-7.329-3.28-7.329-7.328v-34.433c0-4.048 3.281-7.329 7.329-7.329h128.205c4.048 0 7.329 3.281 7.329 7.329v34.433c0 4.048-3.281 7.328-7.329 7.328h-27.179l19.324 105.858h3.178c1.832 0 3.601.687 4.953 1.93l29.87 27.385c2.983 2.731 3.183 7.369.447 10.352-2.731 2.977-7.369 3.189-10.352.447l-27.768-25.456h-6.441c-3.538 0-6.573-2.531-7.208-6.012l-21.997-120.516c-.389-2.141.189-4.339 1.581-6.012 1.391-1.666 3.452-2.634 5.628-2.634h28.639v-19.776h-113.551v19.776h28.645c2.176 0 4.237.968 5.628 2.634 1.391 1.672 1.97 3.87 1.58 6.012l-22.003 120.516c-.636 3.481-3.67 6.012-7.208 6.012h-6.441l-39.013 35.767h210.01c4.048 0 7.329 3.28 7.329 7.328-.001 4.048-3.282 7.329-7.33 7.329z">
                                        </path>
                                        <path d="m152.696 275.676h-20.6c-4.048 0-7.329-3.28-7.329-7.328s3.281-7.329 7.329-7.329h20.6c4.048 0 7.329 3.28 7.329 7.329-.001 4.047-3.281 7.328-7.329 7.328z">
                                        </path>
                                        <path d="m179.44 396.192h-69.347c-4.048 0-7.329-3.28-7.329-7.329 0-4.048 3.281-7.329 7.329-7.329h69.347c4.048 0 7.329 3.281 7.329 7.329-.001 4.048-3.282 7.329-7.329 7.329z">
                                        </path>
                                        <path d="m454.199 446.616h-228.847c-3.023 0-5.737-1.855-6.83-4.672s-.349-6.017 1.878-8.056l54.999-50.424c1.352-1.243 3.121-1.93 4.953-1.93h3.183l19.323-105.858h-27.184c-4.048 0-7.329-3.28-7.329-7.328v-34.433c0-4.048 3.28-7.329 7.329-7.329h128.205c4.048 0 7.329 3.281 7.329 7.329v34.433c0 4.048-3.28 7.328-7.329 7.328h-27.184l19.323 105.858h3.183c1.833 0 3.602.687 4.953 1.93l54.999 50.424c2.227 2.039 2.972 5.239 1.878 8.056-1.095 2.817-3.808 4.672-6.832 4.672zm-210.01-14.657h191.174l-39.014-35.767h-6.44c-3.538 0-6.573-2.531-7.208-6.012l-22.003-120.516c-.39-2.141.188-4.339 1.58-6.012 1.391-1.666 3.453-2.634 5.629-2.634h28.644v-19.776h-113.549v19.776h28.645c2.176 0 4.237.968 5.628 2.634 1.392 1.672 1.97 3.87 1.58 6.012l-22.003 120.516c-.635 3.481-3.67 6.012-7.208 6.012h-6.441z">
                                        </path>
                                        <path d="m332.241 275.676h-20.595c-4.048 0-7.329-3.28-7.329-7.328s3.28-7.329 7.329-7.329h20.595c4.048 0 7.328 3.28 7.328 7.329 0 4.047-3.28 7.328-7.328 7.328z">
                                        </path>
                                        <path d="m358.99 396.192h-69.346c-4.048 0-7.329-3.28-7.329-7.329 0-4.048 3.28-7.329 7.329-7.329h69.346c4.048 0 7.329 3.281 7.329 7.329s-3.281 7.329-7.329 7.329z">
                                        </path>
                                        <path d="m345.57 241.243h-192.983c-3.024 0-5.737-1.855-6.831-4.672-1.093-2.817-.35-6.017 1.878-8.056l46.376-42.523c1.352-1.243 3.121-1.93 4.953-1.93h1.724l15.877-86.981h-21.545c-4.048 0-7.328-3.281-7.328-7.329v-29.04c0-4.047 3.28-7.329 7.328-7.329h108.12c4.048 0 7.329 3.281 7.329 7.329v29.04c0 4.048-3.28 7.329-7.329 7.329h-21.55l15.877 86.981h1.723c1.832 0 3.602.687 4.953 1.93l46.382 42.523c2.228 2.039 2.972 5.239 1.878 8.056-1.095 2.817-3.809 4.672-6.832 4.672zm-174.146-14.657h155.309l-30.396-27.865h-4.981c-3.539 0-6.573-2.531-7.209-6.012l-18.556-101.64c-.39-2.141.188-4.339 1.58-6.012 1.391-1.666 3.453-2.633 5.629-2.633h23.011v-14.382h-93.463v14.382h23.005c2.176 0 4.237.968 5.628 2.633 1.391 1.672 1.97 3.87 1.58 6.012l-18.55 101.639c-.636 3.481-3.67 6.012-7.209 6.012h-4.987z">
                                        </path>
                                        <path d="m242.729 97.082h-17.377c-4.048 0-7.329-3.281-7.329-7.329s3.281-7.329 7.329-7.329h17.377c4.048 0 7.329 3.28 7.329 7.329-.001 4.047-3.281 7.329-7.329 7.329z">
                                        </path>
                                        <path d="m265.281 198.72h-58.48c-4.048 0-7.329-3.281-7.329-7.329s3.28-7.329 7.329-7.329h58.48c4.048 0 7.329 3.281 7.329 7.329s-3.281 7.329-7.329 7.329z">
                                        </path>
                                        <path d="m467.671 446.616h-435.342c-4.048 0-7.329-3.28-7.329-7.329 0-4.048 3.281-7.328 7.329-7.328h435.343c4.048 0 7.329 3.28 7.329 7.328-.001 4.048-3.281 7.329-7.33 7.329z">
                                        </path>
                                    </g>
                                </svg>
                            </div>
                            <div class="rs-category-content">
                                <h5 class="rs-category-title">Lightweight & Flexible</h5>
                                <p class="descrip">Easy to handle, cut, and install. </p>
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-4 col-lg-4 col-md-6">
                        <div class="rs-category-item wow fadeIn" data-wow-delay=".5s">
                            <div class="rs-category-icon">
                                <svg xmlns="http://www.w3.org/2000/svg" id="Layer_2" viewBox="0 0 500 500">
                                    <g>
                                        <path d="m250.015 431.812c-1.224 0-2.441-.285-3.56-.858-43.353-22.264-145.305-92.798-155.707-266.514-.176-2.934 1.321-5.722 3.865-7.2l151.47-88.003c2.423-1.399 5.404-1.399 7.827 0l151.476 88.003c2.544 1.479 4.041 4.266 3.865 7.206-10.408 173.746-112.335 244.256-155.676 266.507-1.119.573-2.336.859-3.56.859zm-143.399-263.525c11.138 157.654 101.093 224.183 143.399 246.934 42.299-22.745 132.225-89.249 143.37-246.934l-143.388-83.304z">
                                        </path>
                                        <path d="m249.997 475c-.901 0-1.802-.158-2.661-.469-1.929-.7-192.491-72.974-195.717-330.912-.037-2.812 1.449-5.423 3.877-6.835l190.587-110.734c2.423-1.399 5.404-1.399 7.827 0l190.593 110.734c2.429 1.412 3.913 4.023 3.878 6.835-3.226 257.938-193.794 330.212-195.724 330.912-.859.311-1.759.469-2.66.469zm-182.718-327.042c4.875 226.709 160.289 301.273 182.718 310.846 22.427-9.573 177.847-84.137 182.723-310.846l-182.723-106.163z">
                                        </path>
                                        <path d="m204.665 318.795h-61.721c-4.303 0-8.381-2.1-11.199-5.77-3.518-4.577-4.717-11.107-3.134-17.041l9.488-35.538c1.96-7.31 7.718-12.215 14.334-12.215h42.744c6.615 0 12.374 4.905 14.327 12.203l9.495 35.55c1.582 5.934.383 12.464-3.134 17.041-2.818 3.67-6.896 5.77-11.2 5.77zm-60.759-15.581h59.798c.31-.626.603-1.838.237-3.201 0-.007-9.488-35.55-9.488-35.55-.073-.274-.159-.487-.238-.652h-40.82c-.079.164-.164.384-.244.664l-9.483 35.531c-.364 1.37-.072 2.582.238 3.208z">
                                        </path>
                                        <path d="m280.86 318.795h-61.721c-4.303 0-8.38-2.1-11.199-5.77-3.518-4.577-4.717-11.107-3.134-17.041l9.488-35.538c1.96-7.31 7.717-12.215 14.334-12.215h42.744c6.615 0 12.373 4.905 14.327 12.203l9.495 35.556c1.583 5.928.384 12.458-3.135 17.035-2.818 3.67-6.895 5.77-11.199 5.77zm-60.759-15.581h59.798c.31-.626.602-1.838.237-3.201 0-.007-9.488-35.55-9.488-35.55-.073-.274-.159-.487-.238-.652h-40.821c-.079.164-.165.384-.243.664l-9.483 35.531c-.365 1.37-.073 2.582.238 3.208z">
                                        </path>
                                        <path d="m242.766 263.811h-61.727c-4.297 0-8.375-2.1-11.193-5.757-3.519-4.583-4.724-11.113-3.141-17.042l9.495-35.55c1.947-7.303 7.711-12.215 14.333-12.215h42.738c6.622 0 12.385 4.912 14.334 12.215l9.494 35.544c1.582 5.935.378 12.465-3.14 17.048-2.818 3.657-6.896 5.757-11.193 5.757zm-60.766-15.58h59.804c.304-.627.602-1.838.238-3.208l-9.495-35.544c-.072-.274-.158-.487-.237-.651h-40.815c-.079.164-.165.377-.238.651l-9.494 35.544c-.365 1.364-.067 2.581.237 3.208z">
                                        </path>
                                        <path d="m318.954 263.811h-61.721c-4.304 0-8.381-2.1-11.199-5.769-3.518-4.577-4.717-11.107-3.134-17.042l9.495-35.538c1.947-7.303 7.704-12.215 14.327-12.215h42.744c6.621 0 12.385 4.912 14.333 12.215l9.489 35.538c1.582 5.935.383 12.465-3.134 17.042-2.813 3.669-6.896 5.769-11.2 5.769zm-60.759-15.58h59.804c.304-.627.597-1.838.231-3.202 0-.006-9.488-35.55-9.488-35.55-.073-.274-.159-.487-.238-.651h-40.814c-.079.164-.164.377-.238.651l-9.495 35.544c-.365 1.37-.072 2.581.238 3.208z">
                                        </path>
                                        <path d="m280.86 208.828h-61.721c-4.303 0-8.38-2.1-11.199-5.77-3.518-4.577-4.717-11.107-3.134-17.042l9.488-35.538c1.96-7.316 7.723-12.221 14.334-12.221h42.744c6.609 0 12.373 4.905 14.327 12.209l9.495 35.556c1.583 5.928.384 12.458-3.135 17.035-2.818 3.671-6.895 5.771-11.199 5.771zm-60.759-15.581h59.798c.31-.626.602-1.838.237-3.201 0-.007-9.488-35.55-9.488-35.55-.073-.274-.159-.493-.244-.658h-40.809c-.085.164-.171.383-.249.67l-9.483 35.531c-.365 1.369-.073 2.581.238 3.208z">
                                        </path>
                                        <path d="m357.055 318.795h-61.721c-4.303 0-8.387-2.1-11.199-5.77-3.518-4.577-4.717-11.107-3.134-17.041l9.489-35.538c1.947-7.304 7.711-12.215 14.333-12.215h42.744c6.622 0 12.38 4.911 14.328 12.215l9.494 35.538c1.583 5.934.384 12.464-3.134 17.041-2.82 3.67-6.897 5.77-11.2 5.77zm-60.766-15.581h59.804c.311-.626.602-1.838.238-3.201 0-.007-9.495-35.55-9.495-35.55-.073-.274-.159-.487-.237-.652h-40.815c-.079.164-.164.378-.238.652l-9.488 35.543c-.366 1.37-.073 2.582.231 3.208z">
                                        </path>
                                        <path d="m361.078 318.795h-223.233c-4.303 0-7.791-3.487-7.791-7.791 0-4.303 3.487-7.791 7.791-7.791h223.234c4.303 0 7.79 3.488 7.79 7.791-.001 4.304-3.488 7.791-7.791 7.791z">
                                        </path>
                                        <path d="m261.384 389.95c-2.988 0-5.976-.949-8.587-2.842l-2.799-2.039-2.794 2.033c-5.222 3.797-11.953 3.791-17.176.006-5.221-3.792-7.309-10.207-5.319-16.342l1.077-3.287-2.805-2.038c-5.216-3.785-7.297-10.201-5.301-16.335 1.991-6.136 7.444-10.098 13.896-10.098h3.456l1.071-3.281c1.997-6.141 7.45-10.103 13.896-10.103 6.451 0 11.904 3.963 13.901 10.097l1.071 3.287h3.451c6.451 0 11.911 3.968 13.901 10.103 1.991 6.135-.097 12.544-5.313 16.335l-2.793 2.026 1.065 3.293c1.996 6.129-.085 12.544-5.301 16.336-2.614 1.9-5.603 2.843-8.597 2.849zm-11.387-20.699c3.012 0 6.025.937 8.593 2.805l.956.694-.366-1.133c-1.96-6.038.17-12.599 5.301-16.329l.961-.7h-1.18c-6.349 0-11.936-4.054-13.896-10.091l-.365-1.132-.372 1.126c-1.96 6.044-7.547 10.097-13.895 10.097h-1.187l.961.7c5.137 3.725 7.273 10.286 5.307 16.329l-.365 1.126.949-.688c2.572-1.867 5.585-2.804 8.598-2.804z">
                                        </path>
                                        <path d="m295.285 280.488c-2.489 0-4.943-1.193-6.446-3.409l-11.345-16.676c-2.422-3.56-1.496-8.405 2.058-10.822 3.548-2.422 8.405-1.503 10.821 2.057l11.345 16.677c2.423 3.56 1.497 8.405-2.057 10.821-1.339.914-2.867 1.352-4.376 1.352z">
                                        </path>
                                        <path d="m204.708 280.488c-1.509 0-3.037-.438-4.376-1.352-3.555-2.416-4.479-7.261-2.057-10.821l11.345-16.677c2.416-3.56 7.273-4.479 10.822-2.057 3.555 2.417 4.479 7.261 2.057 10.822l-11.345 16.676c-1.503 2.216-3.957 3.409-6.446 3.409z">
                                        </path>
                                        <path d="m249.461 223.362c-4.303 0-7.791-3.487-7.791-7.791v-14.534c0-4.303 3.488-7.791 7.791-7.791s7.791 3.488 7.791 7.791v14.534c0 4.304-3.488 7.791-7.791 7.791z">
                                        </path>
                                    </g>
                                </svg>
                            </div>
                            <div class="rs-category-content">
                                <h5 class="rs-category-title">High Thermal Efficiency</h5>
                                <p class="descrip">Minimizes heat loss and optimizes energy usage. </p>
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-4 col-lg-4 col-md-6">
                        <div class="rs-category-item wow fadeIn" data-wow-delay=".7s">
                            <div class="rs-category-icon">
                                <svg xmlns="http://www.w3.org/2000/svg" id="Layer_3" viewBox="0 0 500 500">
                                    <g>
                                        <path d="m306.845 474.732h-256.403c-2.435 0-4.73-1.138-6.203-3.074-1.474-1.942-1.958-4.456-1.306-6.8l38.314-138.156v-108.46c0-4.303 3.488-7.792 7.792-7.792h179.207c4.303 0 7.792 3.489 7.792 7.792v108.461l38.315 138.156c.651 2.344.167 4.858-1.306 6.8-1.473 1.935-3.767 3.073-6.202 3.073zm-246.154-15.583h235.905l-35.858-129.305c-.188-.676-.283-1.376-.283-2.082v-101.728h-163.623v101.728c0 .707-.094 1.406-.283 2.082z">
                                        </path>
                                        <path d="m268.247 226.034h-179.206c-7.588 0-13.764-6.173-13.764-13.764v-12.407c0-7.591 6.176-13.77 13.764-13.77h179.207c7.587 0 13.764 6.179 13.764 13.77v12.407c-.001 7.591-6.177 13.764-13.765 13.764zm-177.387-15.584h175.566v-8.772h-175.566z">
                                        </path>
                                        <path d="m268.317 201.677h-175.484c-4.088 0-7.479-3.159-7.771-7.231-.079-1.077-.136-1.96-.136-2.855 0-14.854 8.498-28.38 21.376-35.07-.465-2.691-.7-5.4-.7-8.121 0-26.596 21.635-48.231 48.228-48.231 6.444 0 12.808 1.309 18.725 3.817 6.039-6.648 14.278-10.757 23.3-11.505 8.617-16.315 25.558-26.492 44.533-25.829 3.735-23.425 24.076-41.383 48.536-41.383 23.12 0 42.813 16.016 47.872 38.066 17.392 5.503 29.512 21.678 29.512 40.353 0 21.313-15.569 38.905-36.202 41.888-8.106 13.277-22.731 21.641-38.424 21.641-3.144 0-6.258-.335-9.292-.98.359 2.411.541 4.895.541 7.433 0 9.046-2.906 17.666-8.401 24.922-1.474 1.942-3.775 3.085-6.213 3.085zm-167.156-15.583h162.962c2.118-3.799 3.224-8.036 3.224-12.425 0-7.257-2.109-13.691-5.868-18.147-.673-.609-1.327-1.242-1.966-1.893-.548-.401-1.05-.889-1.488-1.449-2.609-3.348-1.97-8.23 1.312-10.908.429-.341.758-.56 1.095-.749 3.321-1.887 7.646-1.151 10.045 1.887.727.682 1.427 1.388 2.097 2.131 5.324 4.596 11.992 7.091 19.109 7.091 11.25 0 21.668-6.586 26.539-16.788 1.248-2.612 3.841-4.316 6.736-4.426 14.448-.547 25.768-12.291 25.768-26.73 0-13.002-9.292-24.082-22.094-26.346-3.397-.597-5.993-3.361-6.383-6.788-1.938-16.929-16.263-29.701-33.322-29.701-18.506 0-33.563 15.06-33.563 33.566l.048.823c.043 2.386-1.007 4.656-2.852 6.173-1.844 1.509-4.28 2.093-6.611 1.583-16.303-3.586-32.008 5.272-37.889 20.204-1.242 3.16-4.435 5.181-7.756 4.919-.462-.031-.916-.085-1.37-.147-7.244 0-13.678 3.714-17.379 9.923-1.065 1.79-2.803 3.08-4.824 3.58-2.015.511-4.155.171-5.932-.92-5.141-3.141-11.006-4.803-16.966-4.803-18.001 0-32.645 14.647-32.645 32.647 0 3.47.573 6.946 1.695 10.318.67 2.009.497 4.2-.483 6.076-.98 1.874-2.681 3.269-4.712 3.865-8.316 2.44-14.564 9.24-16.527 17.434z">
                                        </path>
                                        <path d="m110.804 170.748c-2.913 0-5.705-1.638-7.04-4.438-1.85-3.89-.201-8.54 3.683-10.392 8.622-4.108 18.636-4.924 27.752-2.331 4.14 1.169 6.548 5.479 5.372 9.619-1.178 4.145-5.479 6.574-9.621 5.369-5.491-1.546-11.59-1.065-16.801 1.418-1.082.511-2.223.755-3.345.755z">
                                        </path>
                                        <path d="m170.883 137.82c-4.072 0-7.5-3.166-7.767-7.287-.049-.773-.076-1.565-.076-2.356 0-19.748 16.071-35.819 35.821-35.819 8.261 0 16.324 2.879 22.7 8.108 3.327 2.728 3.814 7.64 1.087 10.97-2.727 3.324-7.643 3.805-10.967 1.084-3.652-2.995-8.084-4.578-12.82-4.578-11.158 0-20.238 9.077-20.238 20.235 0 .451.015.901.046 1.345.277 4.292-2.976 7.999-7.271 8.28-.17.012-.344.018-.515.018z">
                                        </path>
                                        <path d="m275.12 163.345c-1.166 0-2.35-.262-3.466-.816-12.979-6.459-22.104-18.841-24.408-33.11-.689-4.249 2.2-8.249 6.45-8.937 4.261-.675 8.248 2.204 8.933 6.453 1.506 9.326 7.475 17.417 15.97 21.641 3.85 1.918 5.421 6.593 3.503 10.446-1.364 2.74-4.118 4.323-6.982 4.323z">
                                        </path>
                                        <path d="m249.218 93.819c-3.376 0-6.484-2.209-7.479-5.613-1.303-4.468-1.964-9.106-1.964-13.788 0-27.101 22.049-49.15 49.148-49.15s49.147 22.049 49.147 49.15c0 4.243-.545 8.468-1.619 12.558-1.096 4.158-5.379 6.648-9.518 5.552-4.164-1.096-6.647-5.357-5.555-9.521.736-2.788 1.108-5.68 1.108-8.59 0-18.506-15.057-33.566-33.564-33.566-18.506 0-33.563 15.06-33.563 33.566 0 3.208.45 6.374 1.342 9.429 1.205 4.128-1.169 8.456-5.299 9.661-.727.215-1.463.312-2.184.312z">
                                        </path>
                                        <path d="m323.935 146.056c-6.626 0-13.228-1.57-19.093-4.535-3.838-1.942-5.378-6.629-3.433-10.47 1.945-3.835 6.632-5.375 10.471-3.433 3.743 1.893 7.801 2.855 12.056 2.855 4.304 0 7.792 3.488 7.792 7.791-.001 4.304-3.488 7.792-7.793 7.792z">
                                        </path>
                                        <path d="m453.27 474.732h-420.478c-4.304 0-7.792-3.488-7.792-7.792 0-4.303 3.488-7.791 7.792-7.791h412.686v-179.35h-177.231c-4.304 0-7.792-3.489-7.792-7.792 0-4.304 3.488-7.792 7.792-7.792h185.023c4.303 0 7.792 3.488 7.792 7.792v194.933c.001 4.304-3.488 7.792-7.792 7.792z">
                                        </path>
                                        <path d="m467.208 474.732h-13.938c-4.303 0-7.792-3.488-7.792-7.792 0-4.303 3.489-7.791 7.792-7.791h13.938c4.303 0 7.792 3.488 7.792 7.791 0 4.304-3.489 7.792-7.792 7.792z">
                                        </path>
                                        <path d="m467.208 279.799h-13.938c-4.303 0-7.792-3.489-7.792-7.792 0-4.304 3.489-7.792 7.792-7.792h6.145v-7.184h-191.168c-4.304 0-7.792-3.488-7.792-7.791 0-4.304 3.488-7.792 7.792-7.792h198.96c4.303 0 7.792 3.488 7.792 7.792v22.767c.001 4.303-3.488 7.792-7.791 7.792z">
                                        </path>
                                        <path d="m344.782 368.311h-37.937c-4.303 0-7.792-3.489-7.792-7.792 0-4.304 3.489-7.792 7.792-7.792h30.146v-38.083h-40.065c-4.303 0-7.792-3.488-7.792-7.792 0-4.303 3.489-7.792 7.792-7.792h47.856c4.304 0 7.792 3.489 7.792 7.792v53.668c0 4.302-3.488 7.791-7.792 7.791z">
                                        </path>
                                        <path d="m427.715 369.705h-37.938c-4.303 0-7.792-3.488-7.792-7.792 0-4.303 3.489-7.792 7.792-7.792h30.146v-38.083h-40.062c-4.304 0-7.792-3.489-7.792-7.792 0-4.304 3.488-7.792 7.792-7.792h47.854c4.303 0 7.791 3.488 7.791 7.792v53.667c.001 4.304-3.487 7.792-7.791 7.792z">
                                        </path>
                                        <path d="m399.837 474.732c-4.303 0-7.791-3.488-7.791-7.792v-60.996h-40.635v60.996c0 4.304-3.488 7.792-7.792 7.792-4.303 0-7.792-3.488-7.792-7.792v-68.788c0-4.303 3.489-7.792 7.792-7.792h56.218c4.304 0 7.792 3.489 7.792 7.792v68.788c0 4.304-3.488 7.792-7.792 7.792z">
                                        </path>
                                        <path d="m221.891 257.031h-132.85c-4.304 0-7.792-3.488-7.792-7.791 0-4.304 3.488-7.792 7.792-7.792h132.85c4.303 0 7.792 3.488 7.792 7.792 0 4.303-3.489 7.791-7.792 7.791z">
                                        </path>
                                    </g>
                                </svg>
                            </div>
                            <div class="rs-category-content">
                                <h5 class="rs-category-title">Chemical & Corrosion Resistant </h5>
                                <p class="descrip"> Performs well in harsh environments. </p>
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-4 col-lg-4 col-md-6">
                        <div class="rs-category-item wow fadeIn" data-wow-delay=".9s">
                            <div class="rs-category-icon">
                                <svg xmlns="http://www.w3.org/2000/svg" id="Layer_4" viewBox="0 0 500 500">
                                    <g>
                                        <path d="m168.653 455.801h-110.765c-6.045 0-11.718-2.965-15.56-8.139-4.577-6.153-5.935-14.367-3.647-21.973l17.026-56.584c2.779-9.238 10.502-15.445 19.212-15.445h76.701c8.705 0 16.427 6.207 19.207 15.445l17.032 56.584c2.289 7.611.919 15.825-3.653 21.978-3.841 5.169-9.514 8.134-15.553 8.134zm-93.733-87.528c-2.181 0-4.327 2.075-5.217 5.045l-17.026 56.584c-1.136 3.773-.034 7.146 1.375 9.044.759 1.025 2.044 2.243 3.836 2.243h110.765c1.786 0 3.07-1.219 3.83-2.241 1.41-1.9 2.517-5.276 1.382-9.049l-17.032-56.582c-.89-2.97-3.037-5.045-5.211-5.045h-76.702z">
                                        </path>
                                        <path d="m305.382 455.801h-110.764c-6.044 0-11.717-2.967-15.559-8.139-4.572-6.153-5.936-14.369-3.647-21.973l17.032-56.584c2.78-9.238 10.503-15.445 19.207-15.445h76.701c8.705 0 16.427 6.207 19.207 15.445l17.031 56.584c2.289 7.606.925 15.82-3.647 21.973-3.842 5.172-9.516 8.139-15.561 8.139zm-93.732-87.528c-2.174 0-4.32 2.075-5.211 5.045l-17.031 56.584c-1.136 3.771-.034 7.144 1.381 9.044.759 1.021 2.038 2.243 3.83 2.243h110.765c1.793 0 3.071-1.222 3.83-2.243 1.415-1.9 2.517-5.274 1.381-9.044l-17.032-56.584c-.89-2.97-3.036-5.045-5.211-5.045z">
                                        </path>
                                        <path d="m237.015 368.273h-110.765c-6.038 0-11.712-2.966-15.553-8.134-4.572-6.153-5.942-14.366-3.653-21.975l17.032-56.584c2.78-9.238 10.497-15.445 19.207-15.445h76.707c8.71 0 16.427 6.207 19.206 15.448l17.026 56.578c2.289 7.605.925 15.821-3.647 21.972-3.842 5.174-9.515 8.14-15.56 8.14zm-93.733-87.526c-2.174 0-4.32 2.076-5.211 5.043l-17.032 56.584c-1.136 3.771-.029 7.146 1.381 9.047.759 1.021 2.044 2.24 3.83 2.24h110.766c1.792 0 3.076-1.219 3.835-2.243 1.41-1.898 2.512-5.271 1.375-9.044l-17.026-56.581c-.89-2.971-3.036-5.046-5.211-5.046z">
                                        </path>
                                        <path d="m373.75 368.273h-110.765c-6.044 0-11.718-2.968-15.559-8.14-4.572-6.153-5.936-14.369-3.648-21.972l17.026-56.581c2.791-9.241 10.508-15.445 19.212-15.445h76.701c8.71 0 16.427 6.207 19.206 15.448l17.032 56.578c2.289 7.611.919 15.824-3.653 21.977-3.839 5.169-9.513 8.135-15.552 8.135zm-93.733-87.526c-2.209 0-4.309 2.029-5.223 5.052l-17.021 56.575c-1.136 3.771-.034 7.144 1.382 9.044.759 1.022 2.037 2.243 3.83 2.243h110.765c1.787 0 3.071-1.219 3.83-2.24 1.41-1.901 2.517-5.277 1.381-9.05l-17.032-56.578c-.89-2.971-3.037-5.046-5.211-5.046z">
                                        </path>
                                        <path d="m305.382 280.747h-110.764c-6.044 0-11.717-2.967-15.559-8.139-4.572-6.153-5.936-14.369-3.647-21.972l17.032-56.584c2.78-9.238 10.503-15.445 19.207-15.445h76.701c8.705 0 16.427 6.207 19.207 15.445l17.031 56.584c2.289 7.606.925 15.82-3.647 21.972-3.842 5.172-9.516 8.139-15.561 8.139zm-93.732-87.528c-2.174 0-4.32 2.075-5.211 5.045l-17.031 56.584c-1.136 3.77-.034 7.144 1.381 9.044.759 1.021 2.038 2.243 3.83 2.243h110.765c1.793 0 3.071-1.222 3.83-2.243 1.415-1.901 2.517-5.274 1.381-9.044l-17.032-56.584c-.89-2.971-3.036-5.045-5.211-5.045z">
                                        </path>
                                        <path d="m442.112 455.801h-110.759c-6.045 0-11.718-2.965-15.559-8.136-4.572-6.15-5.942-14.363-3.653-21.972l17.032-56.587c2.78-9.238 10.502-15.445 19.207-15.445h76.706c8.705 0 16.427 6.207 19.206 15.445l17.032 56.584c2.289 7.611.919 15.825-3.653 21.975-3.84 5.171-9.515 8.136-15.559 8.136zm-93.732-87.528c-2.175 0-4.321 2.075-5.211 5.045l-17.032 56.584c-1.136 3.771-.029 7.146 1.381 9.044.765 1.025 2.043 2.243 3.836 2.243h110.759c1.792 0 3.07-1.219 3.835-2.243 1.41-1.898 2.517-5.274 1.382-9.047l-17.032-56.582c-.89-2.97-3.037-5.045-5.211-5.045h-76.707z">
                                        </path>
                                        <path d="m467.695 455.801h-435.389c-4.035 0-7.306-3.27-7.306-7.305 0-4.036 3.271-7.306 7.306-7.306h435.389c4.035 0 7.305 3.27 7.305 7.306 0 4.035-3.27 7.305-7.305 7.305z">
                                        </path>
                                        <path d="m454.013 351.629c-.154 0-.314-.006-.474-.014-4.024-.257-7.078-3.73-6.82-7.757.925-14.535-.543-30.599-4.378-47.928-6.576 9.775-15.987 22.3-25.987 30.799-2.369 2.024-5.776 2.312-8.47.708-2.683-1.602-4.053-4.732-3.408-7.791.189-.885 16.969-82.866-14.852-139.971-5.588 15.468-16.427 38.356-34.03 48.524-2.454 1.421-5.513 1.284-7.842-.345-2.329-1.63-3.499-4.461-3.003-7.257.143-.825 12.7-74.824-20.308-120.514-.714 15.737-4.201 37.954-16.473 56.895-1.758 2.72-5.086 3.958-8.214 3.025-3.111-.925-5.234-3.79-5.223-7.032 0-.69-.262-67.489-44.532-93.156-44.4 25.727-44.538 92.46-44.532 93.148.018 3.245-2.106 6.113-5.211 7.043-3.128.928-6.461-.303-8.225-3.028-12.272-18.941-15.759-41.158-16.473-56.895-33.008 45.69-20.451 119.689-20.309 120.514.497 2.796-.673 5.628-3.002 7.257-2.329 1.63-5.389 1.766-7.843.345-17.603-10.169-28.442-33.056-34.03-48.524-31.821 57.106-15.04 139.087-14.851 139.971.645 3.059-.725 6.19-3.408 7.791s-6.09 1.315-8.47-.708c-10-8.499-19.412-21.025-25.987-30.799-3.835 17.329-5.303 33.393-4.378 47.928.257 4.027-2.796 7.5-6.821 7.757-4.075.263-7.499-2.799-7.757-6.826-1.324-20.688 1.747-43.984 9.115-69.238.828-2.826 3.265-4.877 6.187-5.211 2.985-.309 5.76 1.119 7.197 3.684.109.183 7.084 12.532 16.53 24.814-3.453-33.85-4.11-96.436 29.207-140.645 1.701-2.26 4.583-3.325 7.34-2.748 2.774.586 4.96 2.726 5.605 5.486.08.323 6.022 25.168 18.727 42.28-2.146-31.559-.337-92.788 39.926-128.02 2.283-2 5.548-2.377 8.236-.95 2.677 1.419 4.195 4.349 3.824 7.357-.034.283-2.46 20.934 3.288 43.043 4.983-25.685 18.133-62.026 52.802-79.649 2.078-1.056 4.544-1.056 6.621 0 34.669 17.623 47.82 53.964 52.803 79.649 5.748-22.109 3.321-42.76 3.287-43.043-.371-3.008 1.148-5.939 3.825-7.357 2.682-1.426 5.942-1.05 8.236.95 40.262 35.232 42.072 96.461 39.925 128.02 12.706-17.112 18.648-41.957 18.728-42.28.645-2.76 2.831-4.9 5.605-5.486 2.78-.591 5.633.492 7.34 2.748 33.316 44.21 32.66 106.795 29.207 140.645 9.446-12.283 16.421-24.631 16.529-24.814 1.439-2.565 4.224-4.018 7.197-3.684 2.922.334 5.36 2.385 6.187 5.211 7.369 25.254 10.44 48.55 9.115 69.238-.241 3.869-3.454 6.84-7.278 6.84z">
                                        </path>
                                    </g>
                                </svg>
                            </div>
                            <div class="rs-category-content">
                                <h5 class="rs-category-title">Durable & Long-lasting</h5>
                                <p class="descrip"> Withstands repeated thermal cycling. </p>
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-4 col-lg-4 col-md-6">
                        <div class="rs-category-item wow fadeIn" data-wow-delay="1.1s">
                            <div class="rs-category-icon">
                                <svg xmlns="http://www.w3.org/2000/svg" id="Layer_5" viewBox="0 0 500 500">
                                    <g>
                                        <path d="m183.976 364.527c-2.105 0-4.198-.853-5.716-2.547-1.361-1.513-32.925-37.83 3.55-105.862 7.858-14.656 9.745-27.716 5.607-38.815-1.258-3.363-.036-7.144 2.946-9.14 2.982-2.002 6.95-1.675 9.575.762.617.574 9.636 9.11 16.477 24.074 7.712-23.336 22.738-50.659 51.494-57.954 2.915-.726 5.982.279 7.881 2.595 1.905 2.316 2.299 5.529 1.016 8.239-.176.363-16.786 36.649.066 46.007 28.902 16.06 15.449 61.245 14.862 63.163-1.252 4.089-5.571 6.376-9.66 5.129-4.083-1.24-6.388-5.564-5.148-9.654.103-.351 10.114-35.277-7.579-45.1-17.1-9.503-17.511-30.081-13.132-47.726-27.286 21.062-32.101 72.061-32.156 72.659-.363 4.162-3.969 7.24-8.124 7.052-4.168-.224-7.41-3.708-7.331-7.882.205-10.506-2.033-19.64-5.045-27.093-1.536 6.775-4.234 13.78-8.105 21.001-31.031 57.882-6.697 87.025-5.644 88.235 2.783 3.212 2.487 8.093-.701 10.906-1.473 1.31-3.306 1.951-5.133 1.951z">
                                        </path>
                                        <path d="m306.551 364.544c-1.7 0-3.411-.556-4.845-1.706-3.333-2.68-3.864-7.555-1.186-10.888 30.233-37.623 18.346-80.426 12.528-95.748-14.517 5.021-17.802 15.685-17.947 16.193-1.192 4.059-5.431 6.472-9.514 5.347-4.083-1.131-6.515-5.275-5.438-9.37.248-.949 6.412-23.264 35.936-28.962 3.473-.641 6.859 1.053 8.408 4.162 1.289 2.607 31.182 64.457-11.898 118.08-1.532 1.901-3.776 2.892-6.044 2.892z">
                                        </path>
                                        <path d="m421.213 394.898h-342.427c-4.276 0-7.743-3.466-7.743-7.743v-30.353c0-4.276 3.466-7.743 7.743-7.743h342.427c4.277 0 7.743 3.466 7.743 7.743v30.353c0 4.277-3.466 7.743-7.743 7.743zm-334.684-15.486h326.941v-14.868h-326.941z">
                                        </path>
                                        <path d="m454.663 473.805h-409.326c-4.277 0-7.743-3.466-7.743-7.742v-78.908c0-4.276 3.466-7.743 7.743-7.743h409.327c4.276 0 7.743 3.466 7.743 7.743v78.908c-.001 4.276-3.467 7.742-7.744 7.742zm-401.585-15.485h393.842v-63.423h-393.842z">
                                        </path>
                                        <path d="m454.663 473.805h-409.326c-4.277 0-7.743-3.466-7.743-7.742v-39.457c0-4.277 3.466-7.743 7.743-7.743h409.327c4.276 0 7.743 3.466 7.743 7.743v39.457c-.001 4.276-3.467 7.742-7.744 7.742zm-401.585-15.485h393.842v-23.972h-393.842z">
                                        </path>
                                        <path d="m444.828 394.898h-389.656c-4.277 0-7.743-3.466-7.743-7.743v-321.34c0-4.276 3.466-7.742 7.743-7.742h389.656c4.276 0 7.743 3.466 7.743 7.742v321.34c0 4.277-3.467 7.743-7.743 7.743zm-381.913-15.486h374.171v-305.855h-374.171z">
                                        </path>
                                        <path d="m399.963 73.557h-299.92c-4.277 0-7.743-3.466-7.743-7.743v-31.876c0-4.277 3.466-7.743 7.743-7.743h299.921c4.277 0 7.743 3.466 7.743 7.743v31.877c-.001 4.277-3.467 7.742-7.744 7.742zm-292.178-15.484h284.436v-16.393h-284.436z">
                                        </path>
                                        <path d="m366.507 364.544h-233.015c-4.276 0-7.743-3.466-7.743-7.743v-117.014c0-68.516 55.741-124.256 124.25-124.256s124.249 55.74 124.249 124.256v117.015c.001 4.277-3.464 7.742-7.741 7.742zm-225.272-15.485h217.53v-109.272c0-59.975-48.79-108.771-108.765-108.771s-108.765 48.796-108.765 108.771z">
                                        </path>
                                        <path d="m467.257 473.805h-434.514c-4.277 0-7.743-3.466-7.743-7.742 0-4.277 3.466-7.743 7.743-7.743h434.515c4.276 0 7.743 3.466 7.743 7.743-.001 4.276-3.468 7.742-7.744 7.742z">
                                        </path>
                                        <path d="m357.682 114.866h-12.927c-4.276 0-7.742-3.466-7.742-7.743s3.466-7.743 7.742-7.743h12.927c4.277 0 7.743 3.466 7.743 7.743-.001 4.276-3.466 7.743-7.743 7.743z">
                                        </path>
                                        <path d="m403.617 146.03h-9c-4.277 0-7.743-3.467-7.743-7.743 0-4.277 3.466-7.743 7.743-7.743h9c4.276 0 7.743 3.466 7.743 7.743 0 4.276-3.467 7.743-7.743 7.743z">
                                        </path>
                                        <path d="m407.313 194.045h-21.698c-4.276 0-7.743-3.466-7.743-7.743 0-4.276 3.467-7.742 7.743-7.742h21.698c4.276 0 7.743 3.466 7.743 7.742 0 4.277-3.467 7.743-7.743 7.743z">
                                        </path>
                                        <path d="m421.213 231.209h-8.13c-4.276 0-7.743-3.466-7.743-7.743 0-4.276 3.467-7.742 7.743-7.742h8.13c4.277 0 7.743 3.466 7.743 7.742 0 4.277-3.466 7.743-7.743 7.743z">
                                        </path>
                                        <path d="m411.009 271.833h-11.893c-4.276 0-7.743-3.466-7.743-7.742 0-4.277 3.467-7.743 7.743-7.743h11.893c4.276 0 7.742 3.466 7.742 7.743 0 4.276-3.466 7.742-7.742 7.742z">
                                        </path>
                                        <path d="m419.084 312.924h-11.771c-4.277 0-7.743-3.466-7.743-7.743s3.466-7.743 7.743-7.743h11.771c4.277 0 7.743 3.466 7.743 7.743-.001 4.276-3.466 7.743-7.743 7.743z">
                                        </path>
                                        <path d="m155.244 114.866h-12.926c-4.277 0-7.743-3.466-7.743-7.743s3.466-7.743 7.743-7.743h12.926c4.277 0 7.743 3.466 7.743 7.743 0 4.276-3.466 7.743-7.743 7.743z">
                                        </path>
                                        <path d="m105.383 146.03h-9c-4.277 0-7.743-3.467-7.743-7.743 0-4.277 3.466-7.743 7.743-7.743h9c4.276 0 7.743 3.466 7.743 7.743 0 4.276-3.466 7.743-7.743 7.743z">
                                        </path>
                                        <path d="m114.384 194.045h-21.697c-4.277 0-7.743-3.466-7.743-7.743 0-4.276 3.466-7.742 7.743-7.742h21.697c4.277 0 7.743 3.466 7.743 7.742-.001 4.277-3.466 7.743-7.743 7.743z">
                                        </path>
                                        <path d="m86.916 231.209h-8.13c-4.276 0-7.743-3.466-7.743-7.743 0-4.276 3.466-7.742 7.743-7.742h8.13c4.277 0 7.743 3.466 7.743 7.742 0 4.277-3.466 7.743-7.743 7.743z">
                                        </path>
                                        <path d="m100.883 271.833h-11.892c-4.276 0-7.743-3.466-7.743-7.742 0-4.277 3.466-7.743 7.743-7.743h11.892c4.277 0 7.743 3.466 7.743 7.743-.001 4.276-3.466 7.742-7.743 7.742z">
                                        </path>
                                        <path d="m92.687 312.924h-11.772c-4.276 0-7.743-3.466-7.743-7.743s3.466-7.743 7.743-7.743h11.771c4.276 0 7.743 3.466 7.743 7.743.001 4.276-3.466 7.743-7.742 7.743z">
                                        </path>
                                        <path d="m259.26 102.574h-18.522c-4.276 0-7.743-3.466-7.743-7.743 0-4.276 3.467-7.743 7.743-7.743h18.522c4.277 0 7.743 3.466 7.743 7.743s-3.466 7.743-7.743 7.743z">
                                        </path>
                                        <path d="m118.54 473.805c-4.276 0-7.743-3.466-7.743-7.742v-39.457c0-4.277 3.466-7.743 7.743-7.743s7.743 3.466 7.743 7.743v39.457c-.001 4.276-3.466 7.742-7.743 7.742z">
                                        </path>
                                        <path d="m188.712 434.349c-4.276 0-7.743-3.466-7.743-7.743v-39.451c0-4.276 3.466-7.743 7.743-7.743s7.743 3.466 7.743 7.743v39.451c0 4.276-3.465 7.743-7.743 7.743z">
                                        </path>
                                        <path d="m248.04 473.805c-4.277 0-7.743-3.466-7.743-7.742v-39.457c0-4.277 3.466-7.743 7.743-7.743 4.276 0 7.743 3.466 7.743 7.743v39.457c0 4.276-3.467 7.742-7.743 7.742z">
                                        </path>
                                        <path d="m318.673 434.349c-4.277 0-7.743-3.466-7.743-7.743v-39.451c0-4.276 3.466-7.743 7.743-7.743 4.276 0 7.743 3.466 7.743 7.743v39.451c-.001 4.276-3.467 7.743-7.743 7.743z">
                                        </path>
                                        <path d="m385.155 473.805c-4.276 0-7.742-3.466-7.742-7.742v-39.457c0-4.277 3.466-7.743 7.742-7.743 4.277 0 7.743 3.466 7.743 7.743v39.457c0 4.276-3.466 7.742-7.743 7.742z">
                                        </path>
                                    </g>
                                </svg>
                            </div>
                            <div class="rs-category-content">
                                <h5 class="rs-category-title"> Safe & Compliant</h5>
                                <p class="descrip">Meets industrial safety standards.</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!-- category area end -->

         <!-- work process area start -->
        <section class="rs-work-process-area section-space rs-work-step-two has-theme-orange-two">
            <div class="rs-work-step-bg-thumb" data-background="assets/images/bg/work-process-03.png"></div>
            <div class="container">
                <div class="row align-items-center g-5">
                    <div class="col-xl-7 col-lg-7">
                        <div class="rs-work-process-content-wrapper">
                            <div class="rs-section-title-wrapper section-title-space">
                                <span class="rs-section-subtitle has-theme-orange justify-content-start">
                           <svg xmlns="http://www.w3.org/2000/svg" width="11" height="15" viewBox="0 0 11 15"
                              fill="none">
                              <path d="M3.14286 10L0 15L8.78104e-07 0L3.14286 5V10Z" fill="#EA5501"></path>
                              <path fill-rule="evenodd" clip-rule="evenodd"
                                 d="M6.28571 10L3.14286 15L3.14286 10L4.71428 7.5L3.14286 5L3.14286 0L6.28571 5L6.28571 10ZM6.28571 10L7.85714 7.5L6.28571 5V0L11 7.5L6.28571 15V10Z"
                                 fill="#EA5501"></path>
                           </svg>
                           Technical Specifications
                        </span>
                                <h2 class="rs-section-title rs-split-text-enable split-in-fade"> Engineered for precise performance</h2>
                            </div>
                            <div class="rs-work-step-wrapper">
                                <div class="rs-work-step-item wow fadeInUp" data-wow-delay=".3s" data-wow-duration="1s">
                                     <p>The Apple Cera Wool Ceramic Fiber Blanket is designed for industrial applications that require precision, safety, and energy efficiency. Below are the key specifications to help you select the right insulation for your project.</p>
                                </div>
                                <div class="container">

                                  <div class="table-responsive">
                                    <table class="table table-bordered table-striped align-middle">
                                      <thead class="table-primary">
                                        <tr>
                                          <th scope="col">Specification</th>
                                          <th scope="col">Value</th>
                                        </tr>
                                      </thead>
                                      <tbody>
                                        <tr>
                                          <td>Thickness</td>
                                          <td>25–100 mm</td>
                                        </tr>
                                        <tr>
                                          <td>Density</td>
                                          <td>128–160 kg/m³</td>
                                        </tr>
                                        <tr>
                                          <td>Max Service Temperature</td>
                                          <td>1600°C</td>
                                        </tr>
                                        <tr>
                                          <td>Thermal Conductivity</td>
                                          <td>0.11–0.16 W/m·K</td>
                                        </tr>
                                        <tr>
                                          <td>Dimensions</td>
                                          <td>610 mm x 6100 mm roll</td>
                                        </tr>
                                      </tbody>
                                    </table>
                                  </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!-- work process area end -->

        <!-- feature area start -->
        <section class="rs-feature-area section-space rs-feature-two">
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-xl-8 col-lg-9 col-md-10">
                        <div class="rs-section-title-wrapper text-center section-title-space">
                            <span class="rs-section-subtitle has-theme-blue">
                        Related Products    
                     </span>
                            <h2 class="rs-section-title has-theme-blue rs-split-text-enable split-in-fade">Complementary insulation solutions</h2>
                        </div>
                    </div>
                </div>
                <div class="row g-5">
                    <div class="col-xl-4 col-lg-4 col-md-6">
                        <div class="rs-feature-item wow fadeInUp" data-wow-delay=".3s" data-wow-duration="1s">
                            <h3 class="rs-feature-title">Ceramic Fiber Blanket </h3>
                            <div class="rs-feature-descrip">
                                <p>Flexible, lightweight insulation designed for high-temperature applications up to 1600°C. Ideal for furnaces, kilns, and industrial ovens, providing excellent energy efficiency and safety.</p>
                            </div>
                            <div class="rs-feature-icon">
                                <img src="assets/images/icon/Ceramic-Fiber-Blanket.png" alt="image">
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-4 col-lg-4 col-md-6">
                        <div class="rs-feature-item wow fadeInUp" data-wow-delay=".5s" data-wow-duration="1s">
                            <h3 class="rs-feature-title">Ceramic Boards</h3>
                            <div class="rs-feature-descrip">
                                <p>Rigid boards for structural insulation. Used in furnace walls, panels, and high-temperature linings. Provides durability, low thermal conductivity, and dimensional stability.</p>
                            </div>
                            <div class="rs-feature-icon">
                                <img src="assets/images/icon/Ceramic-Boards.png" alt="image">
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-4 col-lg-4 col-md-6">
                        <div class="rs-feature-item wow fadeInUp" data-wow-delay=".7s" data-wow-duration="1s">
                            <h3 class="rs-feature-title">Ceramic Bulk Fiber </h3>
                            <div class="rs-feature-descrip">
                                <p>Loose ceramic fibers for filling, backfilling, or custom shaping. Offers excellent thermal resistance and conforms to irregular shapes for complex industrial applications.</p>
                            </div>
                            <div class="rs-feature-icon">
                                <img src="assets/images/icon/Ceramic-Bulk-Fiber.png" alt="image">
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-4 col-lg-4 col-md-6">
                        <div class="rs-feature-item wow fadeInUp" data-wow-delay=".7s" data-wow-duration="1s">
                            <h3 class="rs-feature-title">Ceramic Microporous Insulation </h3>
                            <div class="rs-feature-descrip">
                                <p>Ultra-low thermal conductivity material for extreme temperature control. Ideal for energy-efficient applications where space and weight are critical. </p>
                            </div>
                            <div class="rs-feature-icon">
                                <img src="assets/images/icon/Ceramic-Microporous-Insulation.png" alt="image">
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-4 col-lg-4 col-md-6">
                        <div class="rs-feature-item wow fadeInUp" data-wow-delay=".7s" data-wow-duration="1s">
                            <h3 class="rs-feature-title">Ceramic Modules </h3>
                            <div class="rs-feature-descrip">
                                <p>Pre-formed, ready-to-install ceramic insulation components. Used in high-temperature furnace linings, kiln walls, and thermal process equipment. </p>
                            </div>
                            <div class="rs-feature-icon">
                                <img src="assets/images/icon/Ceramic-Modules.png" alt="image">
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-4 col-lg-4 col-md-6">
                        <div class="rs-feature-item wow fadeInUp" data-wow-delay=".7s" data-wow-duration="1s">
                            <h3 class="rs-feature-title">Ceramic Papers & Felts </h3>
                            <div class="rs-feature-descrip">
                                <p>Thin, flexible insulation sheets for wrapping, sealing, or lining applications. Provides high-temperature resistance and superior heat containment. </p>
                            </div>
                            <div class="rs-feature-icon">
                                <img src="assets/images/icon/Ceramic-Papers-Felts.png" alt="image">
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-4 col-lg-4 col-md-6">
                        <div class="rs-feature-item wow fadeInUp" data-wow-delay=".7s" data-wow-duration="1s">
                            <h3 class="rs-feature-title">Ceramic Textiles </h3>
                            <div class="rs-feature-descrip">
                                <p>Woven or stitched ceramic fabrics for gaskets, sleeves, and protective linings. Combines flexibility with high-temperature durability. </p>
                            </div>
                            <div class="rs-feature-icon">
                                <img src="assets/images/icon/Ceramic-Textiles.png" alt="image">
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-4 col-lg-4 col-md-6">
                        <div class="rs-feature-item wow fadeInUp" data-wow-delay=".7s" data-wow-duration="1s">
                            <h3 class="rs-feature-title">Mixes & Coatings </h3>
                            <div class="rs-feature-descrip">
                                <p>Refractory mixes, pastes, and coatings for surface protection, patching, and thermal management. Enhances performance of industrial equipment under extreme conditions. </p>
                            </div>
                            <div class="rs-feature-icon">
                                <img src="assets/images/icon/Mixes-Coatings.png" alt="image">
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-4 col-lg-4 col-md-6">
                        <div class="rs-feature-item wow fadeInUp" data-wow-delay=".7s" data-wow-duration="1s">
                            <h3 class="rs-feature-title">Hard Refractory & Brick </h3>
                            <div class="rs-feature-descrip">
                                <p>High-strength bricks and refractory materials for structural insulation, furnace floors, walls, and heavy-duty industrial applications. </p>
                            </div>
                            <div class="rs-feature-icon">
                                <img src="assets/images/icon/Hard-Refractory-Brick.png" alt="image">
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-4 col-lg-4 col-md-6">
                        <div class="rs-feature-item wow fadeInUp" data-wow-delay=".7s" data-wow-duration="1s">
                            <h3 class="rs-feature-title">Specialty & Miscellaneous </h3>
                            <div class="rs-feature-descrip">
                                <p>Custom or niche insulation solutions including engineered shapes, specialty fibers, and tailored high-temperature products for unique industrial needs. </p>
                            </div>
                            <div class="rs-feature-icon">
                                <img src="assets/images/icon/Specialty-Miscellaneous.png" alt="image">
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!-- feature area end -->

    </main>
    <!-- Body main wrapper end -->

 
<?php
include __DIR__ . '/A_Layout/Footer/footer.php';
?>